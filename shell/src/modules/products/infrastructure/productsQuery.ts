...
export const getProductsQueryKey = (params: IQueryParams = defaultParams) => ['products', params]

const getProductsQuery = (params: IQueryParams = defaultParams) => ({
  queryKey: getProductsQueryKey(params),
  queryFn: (): Promise<ICollection> =>
    httpService.get<IProductDto[]>(buildUrl('products', params)).then(res => ({
      products: res,
      meta: {
        ...params,
        total: 20,
...
export const useProductsQuery = (params: IQueryParams = defaultParams, options?: UseQueryOptions<ICollection>) => {
  return useQuery({
