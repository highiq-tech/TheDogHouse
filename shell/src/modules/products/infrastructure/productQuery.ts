...
export const getProductQuery = (productId: string) => ({
  ...
  queryFn: (): Promise<IProduct> => httpService.get<IProductDto>(`products/${productId}`),
})

export const useProductQuery = (productId: string, options?: UseQueryOptions<IProduct>) => {
  return useQuery({
...
export const productLoader = async (productId: string) => queryClient.ensureQueryData(getProductQuery(productId))
