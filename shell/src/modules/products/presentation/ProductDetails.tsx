...
const ProductDetails = ({ product, onBack }: IProps) => {
  ...
  return (
    <SimpleGrid w='100%' maxW='1000px' columns={{ base: 1, lg: 2 }} gap={{ base: 6, md: 8 }}>
      <ProductAddedDialog />
      <GridItem colSpan={1}>
        ...
        <VStack spacing={{ base: 1, lg: 3 }} w='100%' align='start'>
          <PageHeader
            ...
            description={t('A part of out {category} collection.', {
              category: <chakra.span fontStyle='italic'>{categoryLabel}</chakra.span>,
            })}
          />
          ...
          <Text color={secondaryColor} fontSize={{ base: 'md', md: 'lg' }} py={{ base: 4, md: 6 }}>
            {product.description}
          </Text>
          <VStack w='100%'>
            <AddToCartButton productId={product.id} colorScheme='teal' />
