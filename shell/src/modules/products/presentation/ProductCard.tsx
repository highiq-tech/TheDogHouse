  ...
  return (
    <VStack spacing={3} overflow='hidden' rounded='lg'>
      ...
      <VStack w='100%' spacing={0} align='flex-start'>
        <HStack w='100%' justify='space-between' fontSize={{ base: 'md', md: 'lg' }} fontWeight='semibold' spacing={6}>
          <Text isTruncated onClick={() => navigate(`/products/${id}`)} cursor='pointer'>
            {title}
          </Text>
          <Text>{moneyVO.format(price)}</Text>
        </HStack>
        <Text fontStyle='italic' fontSize={{ base: 'sm', md: 'md' }} color={categoryColor}>
          {categoryLabel}
        </Text>
      </VStack>
