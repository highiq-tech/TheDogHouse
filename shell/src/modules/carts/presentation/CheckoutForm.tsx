...
const CheckoutForm = ({ onSuccess }: IProps) => {
  ...
  return (
    <VStack
      ...
      <TextInput id='fullname' value={fullName} onChange={e => setFullName(e.currentTarget.value)}>
        {t('Full Name')}
      </TextInput>
      <TextInput id='address' value={address} onChange={e => setAddress(e.currentTarget.value)}>
        {t('Your address')}
      </TextInput>
      <Select
        id='method'
      ...
      <VStack w='100%' pt={6}>
        <Button isLoading={isLoading} type='submit' colorScheme='teal' w='100%'>
          {t('Purchase')}
        </Button>
      </VStack>
    </VStack>
