...
const CartItem = ({ title, category, price, imageUrl, id, quantity }: IProps) => {
  ...
  return (
    <Stack
      ...
      rounded='lg'
      w='100%'>
      <Stack direction={{ base: 'column', md: 'row' }} spacing={{ base: 2, md: 4 }} align='flex-start'>
        <Box
        ...
        />
        <VStack w='100%' h={{ base: 'auto', md: 40 }} spacing={0} align='flex-start' position='relative'>
          <HStack align='center' position='absolute' bottom={0} left={0} display={{ base: 'none', md: 'flex' }}>
            <CheckIcon color='green.400' />
            <Text fontSize='md'>{t('In stock')}</Text>
          </HStack>
          <HStack w='100%' justify='space-between' fontSize='md' fontWeight='semibold' spacing={6}>
            <Text isTruncated onClick={() => navigate(`/products/${id}`)} cursor='pointer' mb={{ base: 0, md: 2 }}>
              {title}
            </Text>
          </HStack>
